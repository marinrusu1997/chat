...
(
    ...
    last_seen_at        TIMESTAMPTZ NOT NULL CHECK (
                            last_seen_at > created_at AND last_seen_at <= NOW() + INTERVAL :'CLOCK_DRIFT'
                        ) DEFAULT NOW(),
...
(
    ...
    user_id                     INTEGER NOT NULL REFERENCES chatting_device(user_id) ON DELETE CASCADE,
    ...
    created_at                  TIMESTAMPTZ NOT NULL CHECK (created_at <= NOW() + INTERVAL :'CLOCK_DRIFT') DEFAULT NOW(),
    last_refilled_at            TIMESTAMPTZ CHECK (last_refilled_at IS NULL OR (last_refilled_at > created_at AND last_refilled_at <= NOW() + INTERVAL :'CLOCK_DRIFT'))
);
...
(
    ...
    last_refill_ts  TIMESTAMPTZ NOT NULL CHECK (last_refill_ts <= NOW() + INTERVAL :'CLOCK_DRIFT') DEFAULT NOW()
);
...
BEGIN
    ...
    IF OLD.user_id IS DISTINCT FROM NEW.user_id THEN
        RAISE EXCEPTION 'The user_id of a device cannot be changed.';
    END IF;
    IF OLD.name IS DISTINCT FROM NEW.name THEN
        RAISE EXCEPTION 'The name of a device is immutable.';
    END IF;
    IF OLD.fingerprint IS DISTINCT FROM NEW.fingerprint THEN
        RAISE EXCEPTION 'The fingerprint of a device is immutable.';
    END IF;
    IF OLD.created_at IS DISTINCT FROM NEW.created_at THEN
        RAISE EXCEPTION 'The created_at timestamp is immutable.';
    END IF;

    RETURN NEW;
END;
$$;
DROP TRIGGER IF EXISTS trg_chatting_device_ensure_immutability ON chatting_device;
CREATE TRIGGER trg_chatting_device_ensure_immutability
    BEFORE UPDATE OF user_id, name, fingerprint, created_at ON chatting_device
    FOR EACH ROW
...
END;
$$;
DROP TRIGGER IF EXISTS trg_chatting_device_monotonic_last_seen ON chatting_device;
...
END;
$$;
DROP TRIGGER IF EXISTS trg_chatting_device_max_count ON chatting_device;
CREATE TRIGGER trg_chatting_device_max_count
    BEFORE INSERT ON chatting_device
...
CREATE OR REPLACE FUNCTION tgf_device_signal_keys_last_refill_on_insert()
    RETURNS TRIGGER AS $$
BEGIN
    IF NEW.last_refilled_at IS NOT NULL THEN
        RAISE EXCEPTION '"last_refilled_at" (%) column must be NULL on initial insertion of user keys.', NEW.last_refilled_at;
    END IF;

    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
CREATE TRIGGER trg_device_signal_keys_last_refill_on_insert
    BEFORE INSERT ON device_signal_keys
    FOR EACH ROW
EXECUTE FUNCTION tgf_device_signal_keys_last_refill_on_insert();
...
CREATE TRIGGER trg_check_one_time_pre_key_limit
    BEFORE INSERT ON one_time_pre_key
    REFERENCING NEW TABLE AS inserted_rows
...
DROP TYPE IF EXISTS device_pre_key_bundle CASCADE;
...
DROP TYPE IF EXISTS device_base_bundle CASCADE;
...
BEGIN
    ...
    FOR v_bundle_base IN
        SELECT device_id, identity_key, signed_pre_key_id, signed_pre_key, signed_pre_key_signature
        FROM device_signal_keys

...
verify_cluster_health() {
	...
	local cluster_info
	cluster_info=$(redis_cli "$admin_password" -h "$coordinator_node_host" -p "$coordinator_node_port" CLUSTER INFO)
	local node_info
	node_info=$(redis_cli "$admin_password" -h "$coordinator_node_host" -p "$coordinator_node_port" CLUSTER NODES)
	...
	for master_id in $master_ids; do
		...
		replica_count_for_master=$(redis_cli "$admin_password" -h "$coordinator_node_host" -p "$coordinator_node_port" CLUSTER REPLICAS "$master_id" | wc -l)
...
log_cluster_status() {
	...
	local cluster_info
	cluster_info=$(redis_cli "$admin_password" -h "$host" -p "$port" CLUSTER INFO)
	local cluster_nodes
	cluster_nodes=$(redis_cli "$admin_password" -h "$host" -p "$port" CLUSTER NODES)
...
initialize_cluster() {
	...
	for node_addr in $redis_nodes; do
		...
		until redis_cli "${redis_passwords_ref["admin"]}" -h "$host" -p "$port" PING | grep -q "PONG"; do
	...
	local known_nodes
	known_nodes=$(redis_cli "${redis_passwords_ref["admin"]}" -h "$coordinator_node" -p "$port" CLUSTER INFO | grep 'cluster_known_nodes' | cut -d: -f2 | tr -d '\r')
	...
	if [ "$known_nodes" -eq "1" ]; then
		...
		if ! echo "yes" | redis_cli "${redis_passwords_ref["admin"]}" -h "$coordinator_node" -p "$port" --cluster create $redis_nodes --cluster-replicas "$replication_factor"; then
...
startup() {
	...
	shutdown() {
		...
		if ! redis_cli "${redis_passwords["admin"]}" -h "$(hostname)" -p "$port" SHUTDOWN; then

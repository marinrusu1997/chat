...
type ConsumerGroupConfig struct {
	...
	InstanceID           string                                                                          `validate:"min=5,max=50"`
...
var requiredConfigurationOptions = [...]string{
	"ClientID",
	"RequiredAcks",
	"FetchMaxWait",
}
...
func (builder *ConfigurationBuilder) GetOptions() ([]kgo.Opt, error) {
	...
	for _, reqKey := range requiredConfigurationOptions {
		if _, exists := builder.options[reqKey]; !exists {
			builder.error = fmt.Errorf("missing required configuration option '%s' in %s", reqKey, util.GetFunctionName(0))
			return nil, builder.error
		}
	}
...
func (builder *ConfigurationBuilder) SetGeneralConfig(config GeneralConfig) bool {
	...
	return builder.setOption("ClientID", kgo.ClientID(config.ClientID)) &&
		...
		builder.setOption("RequestTimeoutOverhead", kgo.RequestTimeoutOverhead(500*time.Millisecond)) &&
...
func (builder *ConfigurationBuilder) SetConsumerConfig(config ConsumerConfig) bool {
	...
	if !topicsGiven && !partitionsGiven {
		builder.error = fmt.Errorf("invalid configuration: you must provide either ConsumeTopics or ConsumePartitions")
		return false
	}
...
func (builder *ConfigurationBuilder) applyDefaultsAndValidate(config interface{}) bool {
	...
	if v.Kind() != reflect.Ptr || v.IsNil() || v.Elem().Kind() != reflect.Struct {
		builder.error = fmt.Errorf("%s: configuration must be a non-nil pointer to a struct", util.GetFunctionName(0))
		return false
	}

	if err := defaults.Set(config); err != nil {
		typeName := v.Elem().Type().Name()
		builder.error = fmt.Errorf("failed to set %s defaults in %s: %w", typeName, util.GetFunctionName(0), err)
		return false
	}

	if err := validatorG.Struct(config); err != nil {
		typeName := v.Elem().Type().Name()
		builder.error = fmt.Errorf("%s configuration failed validation in %s: %w", typeName, util.GetFunctionName(0), err)
		return false
	}
...
func (builder *ConfigurationBuilder) setOption(key string, opt kgo.Opt) bool {
	if _, exists := builder.options[key]; exists {
		builder.error = fmt.Errorf("option with key %s already exists in %s", key, util.GetFunctionName(0))
		return false
	}
